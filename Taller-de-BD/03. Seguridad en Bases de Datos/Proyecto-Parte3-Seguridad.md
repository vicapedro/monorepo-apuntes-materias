...
- **Parte 2:** Implementación física con DML (carga de datos)
...
|-------|----------------------|---------------|
| Bibliotecario | `rol_bibliotecario` | Operaciones CRUD sobre préstamos, multas, inventario |
| Usuario/Lector | `rol_lector` | Solo lectura de catálogo y datos personales |
| Administrador | `rol_admin_sistema` | Control total sobre usuarios y configuración |
| Contador | `rol_contador` | Lectura de transacciones financieras, generación de reportes |
| Director | `rol_director` | Lectura de todas las tablas para reportes ejecutivos |
...
|-----|-----------------|------------------|-----------------|-----------------|-----------------|
| `rol_bibliotecario` | SELECT, UPDATE | SELECT, INSERT, UPDATE, DELETE | SELECT | SELECT, INSERT, UPDATE | SELECT |
| `rol_lector` | SELECT | SELECT (solo sus préstamos) | SELECT (solo sus datos) | SELECT (solo sus multas) | - |
| `rol_admin_sistema` | CONTROL | CONTROL | CONTROL | CONTROL | CONTROL |
| `rol_contador` | SELECT | SELECT | - | SELECT, INSERT, UPDATE | SELECT |
| `rol_director` | SELECT | SELECT | SELECT | SELECT | SELECT |
...
**Ejemplo detallado - rol_bibliotecario:**
...
```sql
-- Permisos para rol_bibliotecario

-- Permisos sobre schema Catalogo
GRANT SELECT, UPDATE ON SCHEMA::Catalogo TO rol_bibliotecario;

-- Permisos sobre schema Prestamos
GRANT SELECT, INSERT, UPDATE, DELETE ON SCHEMA::Prestamos TO rol_bibliotecario;

-- Permisos sobre schema Usuarios (solo lectura)
GRANT SELECT ON SCHEMA::Usuarios TO rol_bibliotecario;

-- Permisos sobre schema Finanzas
GRANT SELECT, INSERT, UPDATE ON SCHEMA::Finanzas TO rol_bibliotecario;

-- Permisos sobre schema Reportes
GRANT SELECT ON SCHEMA::Reportes TO rol_bibliotecario;
GO

-- Permisos para rol_lector (muy restringidos)

-- Solo lectura de catálogo a través de vista pública
GRANT SELECT ON Reportes.vw_LibrosDisponibles TO rol_lector;

-- Lectura de sus propios préstamos (se implementa con vistas filtradas o RLS)
GRANT SELECT ON Reportes.vw_MisPrestamos TO rol_lector;

-- Lectura de sus propios datos
GRANT SELECT ON Reportes.vw_MiPerfil TO rol_lector;
GO

-- Permisos para rol_admin_sistema (control total)
GRANT CONTROL ON DATABASE::Biblioteca TO rol_admin_sistema;
GO

-- Permisos para rol_contador
GRANT SELECT ON SCHEMA::Finanzas TO rol_contador;
GRANT SELECT ON SCHEMA::Prestamos TO rol_contador;
GRANT SELECT ON SCHEMA::Reportes TO rol_contador;
GO

-- Permisos para rol_director (solo lectura ejecutiva)
GRANT SELECT ON SCHEMA::Catalogo TO rol_director;
GRANT SELECT ON SCHEMA::Prestamos TO rol_director;
GRANT SELECT ON SCHEMA::Usuarios TO rol_director;
GRANT SELECT ON SCHEMA::Finanzas TO rol_director;
GRANT SELECT ON SCHEMA::Reportes TO rol_director;
GO
```
...
|---------|-----------|-----------------|-------------|
| `usr_maria_garcia` | `maria.garcia` | `rol_bibliotecario` | Bibliotecaria turno matutino |
| `usr_juan_lopez` | `juan.lopez` | `rol_bibliotecario` | Bibliotecario turno vespertino |
| `usr_pedro_martinez` | `pedro.martinez` | `rol_lector` | Usuario regular |
| `usr_ana_rodriguez` | `ana.rodriguez` | `rol_lector` | Usuario estudiante |
| `usr_carlos_admin` | `carlos.admin` | `rol_admin_sistema` | Administrador TI |
| `usr_lucia_contador` | `lucia.contador` | `rol_contador` | Contadora general |
| `usr_roberto_director` | `roberto.director` | `rol_director`, `rol_contador` | Director (múltiples roles) |
...
ALTER ROLE rol_bibliotecario ADD MEMBER usr_maria_garcia;
ALTER ROLE rol_bibliotecario ADD MEMBER usr_juan_lopez;

ALTER ROLE rol_lector ADD MEMBER usr_pedro_martinez;
ALTER ROLE rol_lector ADD MEMBER usr_ana_rodriguez;

ALTER ROLE rol_admin_sistema ADD MEMBER usr_carlos_admin;

ALTER ROLE rol_contador ADD MEMBER usr_lucia_contador;

-- Usuario con múltiples roles
ALTER ROLE rol_director ADD MEMBER usr_roberto_director;
ALTER ROLE rol_contador ADD MEMBER usr_roberto_director;
GO
```
...
```sql
-- Conectarse como usr_maria_garcia (rol_bibliotecario)
...
```sql
-- Conectarse como usr_pedro_martinez (rol_lector)
...
```sql
-- Conectarse como usr_lucia_contador (rol_contador)
...
```sql
-- Conectarse como usr_roberto_director (rol_director)
...
```sql
-- Conectarse como usr_carlos_admin (rol_admin_sistema)
...
R: Sí, puede usar Windows Authentication. En ese caso, los logins se crean con `CREATE LOGIN [DOMINIO\Usuario] FROM WINDOWS;`
