...
```mermaid
flowchart LR
    CR["🎯 Causa raíz:<br/>Falta de site survey profesional"]

    subgraph Personas
    P1["Técnico junior configuró AP sin validación"]
    P2["Falta de capacitación en RF planning"]
    end

    subgraph Procesos
    PR1["No hay proceso de site survey"]
    PR2["Sin validación post-instalación"]
    end

    subgraph Equipos
    E1["APs sobrecargados (>50 clientes)"]
    E2["Interferencia con APs de edificio vecino"]
    end

    subgraph Configuración
    C1["Canal WiFi en banda congestionada"]
    C2["Potencia de transmisión alta (overlap)"]
    end
