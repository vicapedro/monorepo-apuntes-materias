...
```mermaid
flowchart TB
    F["F — Fault"] <--> P["P — Performance"]
    F -->|Fallas afectan desempeño| C["C — Configuration"]
    P -->|Métricas detectan problemas| A["A — Accounting"]

    C <--> A
    C -->|Cambios afectan uso de recursos| S["S — Security"]
    A -->|Uso informa capacity planning| S

    S --- X["Protege toda la infraestructura"]
```
