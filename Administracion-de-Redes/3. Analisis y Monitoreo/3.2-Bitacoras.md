...
graph TD
    ...
    C1 -.->|Write| F
    C2 -.->|Index| G
    Sí. En Mermaid, `%% ...` es comentario y **no se muestra**.  
    Si quieres que “Almacenamiento” aparezca visible “a un lado”, reemplaza el placeholder por un nodo etiqueta:

    ```mermaid
        L[Almacenamiento]:::storage
        L -.-> F
        L -.-> G
    ```

    Esto mostrará el texto en el diagrama y lo asociará a `F` y `G`.
...
│                                                                       │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐           │
│  │ Servidor │  │ Router/  │  │ Firewall │  │  App     │           │
│  │  Linux   │  │ Switch   │  │          │  │ (Node.js)│           │
│  │          │  │          │  │          │  │          │           │
│  │ syslog   │  │ syslog   │  │ syslog   │  │ Winston  │           │
│  └────┬─────┘  └────┬─────┘  └────┬─────┘  └────┬─────┘           │
│       │             │             │             │                  │
└───────┼─────────────┼─────────────┼─────────────┼──────────────────┘
        │             │             │             │
        │ UDP 514     │ TCP 514     │ UDP 514    │ HTTP (API)
        │ (syslog)    │ (TLS)       │            │
        │             │             │             │
        ▼             ▼             ▼             ▼
┌─────────────────────────────────────────────────────────────────────┐
│              Recolector / Agregador de Logs                         │
│                                                                       │
│  ┌────────────────┐        ┌────────────────┐                       │
│  │    rsyslog     │        │   Logstash     │                       │
│  │  (syslog srv)  │   ó    │  (ELK Stack)   │                       │
│  └────────┬───────┘        └────────┬───────┘                       │
│           │                         │                                │
│           │ Parsing, filtering,    │ Parsing, enrichment            │
│           │ normalization          │ grok patterns                  │
│           │                         │                                │
└───────────┼─────────────────────────┼────────────────────────────────┘
            │                         │
            │ Write                   │ Index
            ...
            ▼                         ▼
┌────────────────────┐      ┌────────────────────┐
│  Almacenamiento    │      │  Elasticsearch     │
│  - Archivos planos │      │  (Search engine)   │
│  - Comprimidos     │      │  - Full-text       │
│  - Rotados         │      │  - Aggregations    │
└────────────────────┘      └──────────┬─────────┘
                                       │
                                       │ Query DSL
                                       │
